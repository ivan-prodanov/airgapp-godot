extends Spatial
class_name EnergyVehicleGarage2Manager
...
signal vehicle_2_plugged_in_change(plugged_in, animate)
...
func update(data: EnergySiteData, animate = false):
	if data == null: return
	var has_wall_connector_2 = data.has_wall_connector_2()
	
	if vehicle != null:
		vehicle.updateFromEnergySite(data)
	
	var new_shown = data.has_vehicle_2()
...
func setVehiclePluggedIn(plugged_in: bool, animate = false):
	call_deferred("emit_signal", "vehicle_2_plugged_in_change", plugged_in, animate if should_animate else false)
