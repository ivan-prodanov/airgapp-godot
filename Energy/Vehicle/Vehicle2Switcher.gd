...
func getVehicleResourcePath(data: EnergySiteData):
	if data.get_vehicle_2_type() == EnergySiteData.VehicleType.TESLA:
		return product_manager.get_resource_path_for_product(data.get_vehicle_2_data())
...
func updateFromEnergySite(data: EnergySiteData):
	...
	if not data.has_wall_connector_2():
		deleteVehicle()
		return
		
	if not data.has_vehicle_2():
		return
		
	var vehicle_data = data.get_vehicle_2_data()
...
func instantiateVehicle(data: EnergySiteData):
	...
	var vehicle_type = data.get_vehicle_2_type()
	var vehicle_data = data.get_vehicle_2_data()
