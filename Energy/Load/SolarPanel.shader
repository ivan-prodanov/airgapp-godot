...
uniform float shine_factor : hint_range(0.0, 1.0);

const vec3 LIGHT_DIRECTION = vec3(1.0, 1.0, 1.0);

uniform vec4 ultra_highlight_color: hint_color;
...
vec3 shine_albedo(vec3 albedo, float shine) {
	vec3 shine_final_color = mix(shine_color.rgb, ultra_highlight_color.rgb, 1.2 - shine);
	return albedo + (shine_final_color.rgb * shine);
}
...
void fragment() {
	...
	float shine = shine_amount(shine_factor);
	ALBEDO = shine_albedo(albedo, shine) * lightStrength(NORMAL) * 0.5;
	ALPHA = alpha;
}
