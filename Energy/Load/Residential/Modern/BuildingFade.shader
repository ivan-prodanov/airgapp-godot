...
uniform float fade_amount: hint_range(0.2, 0.7) = 0;
uniform float noise_scale: hint_range(0, 1) = 0;
...
void vertex() {
	vec3 sun_direction = mix(vec3(-0.9, -2.2, 4.7), vec3(-0.3, 1.4, 2.5), theme);
	
	float ambient_strength = mix(0.35, 0.81, theme);
	
	float sun_light = dot(NORMAL, (sun_direction * -0.2)) * 0.5 + 0.5;
...
void fragment() {
	...
	float noise = random(UV2);
	float fade_val = smoothstep(fade_amount, fade_amount + 0.1, (1.0 - UV2.y));
