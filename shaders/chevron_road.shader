...
uniform vec4 u_color:hint_color = vec4(0.5);
...
void fragment()
{
    vec2 uv = vec2(v_uv.x, -v_pos.z);
    // offset for zigzag pattern
    float zigzag = sawtooth(uv.x, 1.5); 
    // scrolling/repeating chevron pattern over uv.y
    float chevron = smoothstep(-0.3, 0.3, sin((u_time * 4.0) + (uv.y * 2.0) - zigzag * 3.0));
    ALPHA = chevron * smoothstep(0.8, 0.1, abs(v_uv.x)) * u_opacity * u_color.a * smoothstep(u_span.x, -2.0, -v_pos.z) * (1.0-(-v_pos.z/u_span.y)) ;
	ALBEDO = u_color.rgb;
