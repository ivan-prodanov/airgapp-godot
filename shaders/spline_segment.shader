...
uniform vec4 color : hint_color= vec4(.5, 0.5, 0.5, 1.0);

uniform vec2 u_span = vec2(-5, 100);
uniform float u_dashed = -1.0;
...
void vertex() {
	...
	vec3 worldPos = v_pos.xyz;//(MODEL_MATRIX * v_pos).xyz;
	vec3 cropPos = abs( ( worldPos - u_world_crop_offset) / u_world_crop_unit );
...
void fragment() {
    ...
    float factor = smoothstep(-0.5, -1.0, u_dashed);
    float alpha = step(u_dashed * factor, sin(-v_pos.z * 0.5 + u_distance)); 

	float feather =  smoothstep(0.45, 0.2, abs(v_uv.x));
	
	ALPHA = clamp(color.a * alpha * feather  * smoothstep(u_span.x, -2.0, -v_pos.z) * (1.0-(-v_pos.z/u_span.y)), 0.0, 1.0);
