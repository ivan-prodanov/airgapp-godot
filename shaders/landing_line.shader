...
uniform lowp float u_weight = 0.5;

uniform vec2 u_span = vec2(-10, 100);
uniform float u_distance = 230.9;
...
{
	ALBEDO = u_color.rgb;
    ALPHA =  u_color.a * clamp(0.0, (0.33+( sin(u_distance*0.1+ -v_pos.z*-0.03) )*0.66) * smoothstep(0.65, 0.5, abs(v_uv.x)) * u_opacity * smoothstep(u_span.x, -2.0, -v_pos.z) * (1.0-(-v_pos.z/u_span.y)), 1.0);
