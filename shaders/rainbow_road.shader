...
void fragment() {
	ALPHA = clamp(0.0, smoothstep(1.0, 0.1, abs(v_uv.x)) * u_opacity * smoothstep(u_span.x, -2.0, -v_pos.z) * (1.0-(-v_pos.z/u_span.y)), 1.0) ;
	ALBEDO = hsb2rgb(vec3(u_distance*0.13 + -v_pos.z * 0.02 , 0.5, 1.0)) ;
}
