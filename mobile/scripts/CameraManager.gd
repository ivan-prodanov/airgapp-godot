...
func _ready():
	...
	# Fit-to-screen on tall (portrait) viewports like iOS: keep horizontal FOV so the wide car
	# fills the width instead of being cropped at the sides (Godot's default vertical FOV
	# over-zooms on tall aspects). Harness-independent — it's a camera property.
...
func on_move_camera(data: Dictionary):
	...
	if animated:
		tween.interpolate_property(pivot, "rotation_degrees", null, rotation, duration, transition_type, ease_type)
		tween.interpolate_property(camera, "translation", null, offset, duration, transition_type, ease_type)
		if cam_fov != null: tween.interpolate_property(camera, "fov", null, cam_fov, duration, transition_type, ease_type)
		current_animation_id = animation_id
		tween.start()
	else:
		current_animation_id = null
		tween.remove_all()
