...
func _ready():
	...
	if background_plane != null and OS.get_name() != "iOS":
...
func on_set_env_params(data: Dictionary):
	...
	if animated:
		tween.interpolate_property(environment, "background_sky_rotation_degrees", null, rotation, duration, transition_type, ease_type)
		tween.interpolate_property(environment, "background_energy", null, env_energy, duration, transition_type, ease_type)
		tween.interpolate_property(environment, "ambient_light_energy", null, amb_energy, duration, transition_type, ease_type)
		tween.start()
	else:
		tween.remove_all()
...
func update_theme(theme):
	var background_color = dark_background_color
	...
	if background_plane != null:
		var background_material = background_plane.get_surface_material(0)
...
func update_loading_effect(product_data: ProductData, animated_stop_loading = false):
	...
	if vehicle_data != null and vehicle_data.isLoading():
